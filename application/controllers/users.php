    ...
    function ubah_password() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if ($this->User_Model->get_validate(array('sandi' => 'new_pwd'))) {
                ...
                if ($current_user->sandi == md5($this->User_Model->salt_formatter($this->input->post('old_pwd')))) {
                    $data['sandi'] = md5($this->User_Model->salt_formatter($this->input->post('new_pwd')));
                    $data['sandi_asli'] = $this->User_Model->formatter($this->input->post('new_pwd'));
                    $this->User_Model->update($this->auth->get_user_id(), $data, TRUE);
                    // echo json_encode(array('msgType' => 'success', 'msgValue' => 'Password berhasil diubah'));
            ...
            $template_body = 'templates/responsive/change_password';
            $template_bottom = 'templates/responsive/footer_home';

            $uspass = $this->User_Model->uspass($this->id);
            // var_dump($uspass);die();
            ...
            $this->load->view($template_body, array('aplikasi' => $this->aplikasi, 'unread_message' => $this->unread_message, 'menus' => $this->menus, 'rolename' => $this->auth->get_rolename(), 'nama_user' => $this->auth->get_user_data()->nama, 'uspass' => $uspass));
