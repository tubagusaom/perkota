...
class Sc_penilaian extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('Sc_penilaian_Model');
...
		$view = $this->load->view('shortcut/penilaian', array('id_asesi'=>$users), TRUE);
...
			$rekomendasi_date = array_reverse(explode("/", $this->input->post('rekomendasi_date')));
			$rekomendasi_date = implode('-', $rekomendasi_date);

			$rekomendasi_description = $this->input->post('rekomendasi_description ');

			$data_penilaian = $this->Sc_penilaian_Model->data_asesi($id_asesi,$id_asesi2,$id_tuk);
			//var_dump($data_penilaian);die();
			if(count($data_penilaian) > 0)
			{
				//var_dump($data_penilaian);die();
				foreach($data_penilaian as $key=>$values){
...
						$this->db->update(kode_tbl().'asesi',$data_udpate);
...
			$view = $this->load->view('shortcut/penilaian', array('id_tuk'=>$tuk_grid,'id_asesi'=>$users,'id_asesi2'=>$users2,'data_perangkat'=>$data_perangkat,'pra_asesmen' => array('-Pilih-', 'Lanjut', 'Tidak Lanjut')), TRUE);
