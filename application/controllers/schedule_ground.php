...
class Schedule_ground extends MY_Controller {
    ...
    function index() {
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            $grid = $this->grid->set_properties(array('model' => 'v_schedule_model', 
            'fields' => array('schedule_date','nama','subject_category','program_study','current_program','instruktur_name','plane_name','from_time','to_time','stage','revision'), 'controller' => 'schedule_ground', 'options' => array('id' => 'schedule_ground', 'pagination', 'rownumber')))->load_model()->set_grid();
            $view = $this->load->view('schedule_ground/index', array('grid' => $grid), true);
    ...
    function datagrid() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			{
				$where['siswa_id LIKE'] = '%' . $this->input->post('siswa_id') . '%';
			}
            $where['subject_category IN (1,3) AND id !='] = '';
    ...
    function add() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            echo json_encode(array('msgType' => 'success', 'msgValue' => 
            $this->load->view('schedule_ground/add', array('siswa_grid' => $schedule_grid, 'instruktur_grid' => $instruktur_grid,
                     'batch_grid' => $batch_grid, 'room_grid' => $room_grid
                    , 'plane_grid' => $plane_grid, 'revision' =>array("New","Revise 1","Revise 2","Revise 3")), TRUE)));
        }
    }
    ...
    function delete($id = false) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (sizeof($schedule) == 1) {
                ...
                $view = $this->load->view('schedule_flight/edit', array('data' => $this->schedule_model->get_single($schedule),'siswa_grid' => $schedule_grid, 'instruktur_grid' => $instruktur_grid,
    ...
    function search() {
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            echo json_encode(array('msgType' => 'success', 'msgValue' => $this->load->view('schedule_flight/search', array('siswa_grid' => $schedule_grid), TRUE)));
        } else {
            block_access_method();
        }
    }
