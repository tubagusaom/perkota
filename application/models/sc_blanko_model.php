...
Class Sc_blanko_model extends MY_Model {
	...
	function data_asesi($id_asesi,$id_asesi2,$skema){
		$jenis_user = $this->auth->get_user_data()->jenis_user;
        
		$this->db->select('id,nama_lengkap,email,telp,no_seri');
		$this->db->where('id >=',$id_asesi);
		$this->db->where('id <=',$id_asesi2);
        $this->db->where('skema_sertifikasi',$skema);
        $this->db->where('terbitkan_sertifikat','on');
        $this->db->order_by('no_urut_sertifikat','DESC');
